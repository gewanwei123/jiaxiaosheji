...
export async function POST(request: Request) {
  try {
    ...
    const { name, phone, email, consultType, description } = formData;
    ...
    let info = await transporter.sendMail({
      ...
      text: `
        ...
        电话: ${phone}
        邮箱: ${email || '未提供'}
        咨询类型: ${consultType ? consultTypeMap[consultType] || consultType : '未提供'}
        咨询内容: ${description || '未提供'}
      `,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          <h2 style="color: #3b82f6;">收到来自政策法规页的新咨询</h2>
          <p><strong>姓名:</strong> ${name}</p>
          <p><strong>电话:</strong> ${phone}</p>
          <p><strong>邮箱:</strong> ${email || '未提供'}</p>
          <p><strong>咨询类型:</strong> ${consultType ? consultTypeMap[consultType] || consultType : '未提供'}</p>
          <p><strong>咨询内容:</strong> ${description || '未提供'}</p>
    ...
    // 5. 打印邮件发送信息
    console.log("政策法规页面咨询邮件已发送: %s", info.messageId);
    
    // 6. 返回成功响应
    return NextResponse.json({ 
      success: true, 
      message: '提交成功！我们的专业顾问将在24小时内与您联系。' 
    });
